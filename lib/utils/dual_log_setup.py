...
def setup_logging(
    ...
    log_line_template="%(color_on)s[%(asctime)s @%(filename)s:%(lineno)d]%(color_off)s"
):
    msg = '%(message)s'

    log_line_template = log_line_template + ' ' + colored(msg, 'cyan')
    # log_line_template = colored(log_line_template, 'cyan', attrs=['dark'])
    ...
    if console_log_level is None:
        console_log_level = logfile_log_level
    ...
    console_formatter = LogFormatter(fmt=log_line_template, color=console_log_color, datefmt="%Y-%m-%d %H:%M:%S")
